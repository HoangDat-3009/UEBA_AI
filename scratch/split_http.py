...
input_path = r"c:\Users\admin\Downloads\source code\UEBA\data\r4.2\http.csv"
output_dir = r"c:\Users\admin\Downloads\source code\UEBA\data\r4.2"

def split_csv(input_path, output_dir, num_splits=25):
    ...
    with open(input_path, 'rb') as f:
        ...
        chunk_idx = 1
        out_path = os.path.join(output_dir, f"http_{chunk_idx}.csv")
        ...
        for line in f:
            ...
            if bytes_written_this_chunk >= target_chunk_size and chunk_idx < num_splits:
                out_f.close()
                print(f"Finished http_{chunk_idx}.csv: {bytes_written_this_chunk} bytes")
                chunk_idx += 1
                out_path = os.path.join(output_dir, f"http_{chunk_idx}.csv")
                out_f = open(out_path, 'wb')
                out_f.write(header)
